...
public class UpdateUserService extends BaseService{
    ...
    private UserBO user;
    private static final String CLASS_NAME = "UpdateUserService";

    public UpdateUserService(UserBO user) {
        this.user = user;
    }
    ...
    protected ErrorCode executeImpl() {
        ...
        status = controller.update(user);
        if(ErrorCode.ERROR.equals(status))
        {
            String errorMessage =  " could not update user entry. please check hibernate transaction in the log.";
